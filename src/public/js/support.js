...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const fieldsetsElements = document.querySelectorAll(
    ".support-form__fieldset"
  );
  ...
  prevButtons.forEach((button) => {
    button.addEventListener("click", function () {
      if (currentStep > 1) {
        currentStep--;
        supportForm.style.transform = `translateX(-${
          (currentStep - 1) * 100
        }%)`;
        displayCurrentFieldset(currentStep);
        progressName[currentStep - 1].classList.remove(
          "support-progress__name--active"
        );
        progressBullet[currentStep - 1].classList.remove(
          "support-progress__bullet--active"
        );
      }
    });
  });
